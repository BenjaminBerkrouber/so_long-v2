...
void	init_ghost_ressources(t_game *game)
{
	game->ressources->ghost_left_ptr = load_img(game, "./img/ghost-left.xpm");
	game->ressources->ghost_right_ptr = load_img(game, "./img/ghost-right.xpm");
	game->ressources->ghost_top_ptr = load_img(game, "./img/ghost-top.xpm");
	game->ressources->ghost_bottom_ptr = load_img(game, "./img/ghost-bot.xpm");
	if (!game->ressources->ghost_left_ptr
		|| !game->ressources->ghost_right_ptr
		|| !game->ressources->ghost_top_ptr
		|| !game->ressources->ghost_bottom_ptr)
	{
		ft_putstr_fd("ERROR\nImg ghost\n", 2);
		exit_game(game, -1);
	}
